...
Running the bats-core test suite with `--jobs` exposed failures that the existing parallel CI job did not cover:
...
- Restore the default `SIGINT` disposition in semaphore workers before running a test command.
- Make the internal launcher derive `BATS_LIBEXEC` from its own location.
- Stop exporting `bats_readlinkf` and add regression coverage for recursive Bats invocation without inherited launcher state.

## Why not export `BATS_NUMBER_OF_PARALLEL_JOBS`?

Exporting it when `--jobs` is used, as proposed in #1234, changes the behavior of nested Bats invocations. In the self-test suite it also activates skip guards in the Ctrl-C tests, hiding the failures reported in #1225.

This keeps the existing distinction:

- `--jobs N`: parallelize this invocation.
- `BATS_NUMBER_OF_PARALLEL_JOBS=N`: also configure nested invocations through the inherited environment.
